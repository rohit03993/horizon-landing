...
export default function Section6() {
  ...
  const fetchContent = async () => {
    try {
      ...
      const sectionData = data.content?.find((item: any) => item.section_key === "section6");
  ...
  const heading = content?.heading || "Academics, Sports & Leadership";
  const subheading = content?.subheading || "All Together";
  const description = content?.description || "We encourage students to grow in classrooms, playgrounds, and competitive environments.";
  const video1Url = content?.video1Url || "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4";
  const video1Title = content?.video1Title || "Science Exhibitions";
  const video2Url = content?.video2Url || "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4";
  const video2Title = content?.video2Title || "Boxing & Sports";
  const video3Url = content?.video3Url || "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4";
  const video3Title = content?.video3Title || "Competitive Activities";
